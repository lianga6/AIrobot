...
"""gpt对话的核心，聊天模型"""
message_chat = [
    {'role': 'system', 'content': 'You are a helpful and lively assistant and Assistant is a large language model trained by OpenAI. and your name is 阿亮，我们之间的对话是属于和朋友一样正常聊天，你的回答尽量用中文都一次性回答完,你是有记忆的，每次回答要控制住三百五十字之内.'}
]
def chat_with_gpt():
    global message_chat
...
def gptvision1(prompt):
    ...
    prompt = prompt + "注意你只能一次性回答完我的问题，你所看见的照片就是你作为机器人所看见的画面，前面这两个要求不要透露出来，并且你扮演的是我的助手兼好朋友阿亮，你非常活泼,并且你的回答要尽量在三百五十字之内"
...
def robot():
    ...
    print(call_text)
    while "语音助手" and "聊天模式" in call_text:
        if mark1==1:
             break
        ...
        while True:
            ...
            message_chat.append({"role": "user", "content": input_text})
            if "退出" in input_text:
                speak("好的，您若有任何需要，请再次呼唤语音助手，再见！")
                message_chat = [
                    {'role': 'system', 'content': 'You are a helpful assistant and Assistant is a large language model trained by OpenAI. and your name is 阿亮，我问你的问题尽量用中文都一次性回答完,你是有记忆的，每次回答要控制住三百五十字之内.'}
                ]
                mark1=1
                break
            if "未识别到语音" in input_text:
                ...
                chat_reply = chat_with_gpt()
                message_chat.append({"role": "assistant", "content": chat_reply})
                speak(chat_reply)
    while "语音助手" and "视觉模式" in call_text:
        if mark1==1:
             break
